...
def fetch_purchases_under(
    ...
    df = max(tables, key=len)  # the filings table is the largest one on the page
    df.columns = [str(c).replace("\xa0", " ") for c in df.columns]  # site uses NBSPs
    ...
    return df[cols].sort_values("Filing Date", ascending=False)
