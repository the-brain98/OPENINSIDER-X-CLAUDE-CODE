...
def _session():
    ...
    # Set SEC_CONTACT_EMAIL in .env -- deliberately no hardcoded default, so a
    # fresh clone fails loudly instead of silently making requests under
    # someone else's identity.
    contact = os.environ["SEC_CONTACT_EMAIL"]
    s.headers["User-Agent"] = f"politicians-tracker/0.1 ({contact})"
    return s
